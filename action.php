...
function writePng($user,$title,$body){
  ...
  if(checkTitle($title)){
    ...
    ImageCopyResampled($out, $img, 0,0,0,0, 120,120, $width, $height);
...
function mkfiles(){
  ...
  file_put_contents("data/files.list", $alls);
}
...
function write($user,$title,$body,$type){
  ...
  if(checkTitle($title)){
    ...
    $link->addAttribute('href','../css/gragen.css');
   ...
   if(
	...
	//}
        ...
        $ret = $to->oAuthRequestImage('https://api.twitter.com/1.1/statuses/update_with_media.json','POST', array(
          'status'=> $tweet . ' #MagicalCircleEngine',
